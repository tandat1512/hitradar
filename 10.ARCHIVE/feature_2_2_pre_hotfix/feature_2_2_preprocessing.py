...
def build_histgb_preprocessor(
    ...
    if categorical_cols:
        ...
        categorical_transformers.append(
            ("encoder", OrdinalEncoder(handle_unknown="use_encoded_value", unknown_value=-1, dtype=np.float32), categorical_cols)
        )

    remainder = "passthrough" if not binary_cols else binary_cols
