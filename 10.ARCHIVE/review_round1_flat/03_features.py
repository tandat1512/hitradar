"""Leakage-safe feature engineering shared by notebooks and deployment."""
...
# ``decade`` is derived deterministically from release_year and is part of the
# original cleaned dataset contract, not one of the 13 candidate FE features.
...
MODEL_FEATURES = BASELINE_MODEL_FEATURES + EXPECTED_ENGINEERED_FEATURES
...
class FeatureBuilder(BaseEstimator, TransformerMixin):
    ...
    ``fit`` learns duration quantiles, numeric fallback values and historical
    decade-level audio means from the training split only. ``transform`` then
    applies those frozen statistics to train, test and online inference rows.
    """
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.include_engineered:
            # Cyclic musical-key encoding.
            result["key_sin"] = np.sin(2 * np.pi * (key % 12) / 12.0)
            result["key_cos"] = np.cos(2 * np.pi * (key % 12) / 12.0)

            # Audio interactions and a stable acoustic/energy balance ratio.
            ...
            # Categories are real columns and are one-hot encoded in modeling.py.
            ...
            result["duration_category"] = pd.cut(
                duration,
                bins=[-np.inf, self.duration_thresholds_["q33"], self.duration_thresholds_["q67"], np.inf],
                labels=["short", "standard", "long"],
                include_lowest=True,
            ).astype("string")
            result["tempo_category"] = pd.cut(
                tempo,
                bins=[-np.inf, 90.0, 120.0, 150.0, np.inf],
                labels=["slow", "moderate", "fast", "very_fast"],
                include_lowest=True,
            ).astype("string")

            # Period means were fitted on TRAIN only; unseen periods use train globals.
    ...
    def get_learned_statistics(self) -> dict[str, Any]:
        check_is_fitted(self, ["duration_thresholds_", "fit_row_count_"])
...
def validate_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    """Return the mandatory PASS/FAIL table for all 13 engineered columns."""
    rows = []
    for feature in EXPECTED_ENGINEERED_FEATURES:
        ...
        if exists:
            ...
            if pd.api.types.is_numeric_dtype(series):
                infinite_count = int(np.isinf(series.to_numpy(dtype=float)).sum())
            else:
                infinite_count = 0
            ...
            infinite_count = None
            dtype = "MISSING"
        passed = exists and missing_count == 0 and infinite_count == 0
        rows.append(
            {
                "Feature": feature,
                "Exists": exists,
...
def build_feature_contract() -> dict[str, Any]:
    return {
        ...
        "expected_engineered_features": EXPECTED_ENGINEERED_FEATURES,
